...
import {
  BookingGroupService,
  BookingGroupServiceError,
} from "@/services/bookingGroup.service";
...
async function getAuthenticatedService() {
  ...
  return { service: new BookingGroupService(supabase) };
}

function handleServiceError(error: unknown) {
  if (error instanceof BookingGroupServiceError) {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const groups = await result.service.getPaginated({ ...pagination, status });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const group = await result.service.create(body);
